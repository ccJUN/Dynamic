...
var slipjs = {
	...
	/**
	 * 修复在IOS下滑动时候会卡顿的问题(只对IOS做此种操作,需要业务方判断平台)
	 * @param dom 需要插入伪元素before的CSS选择器，例如可以是div、.myBaner、#myBanner之类的
	 */
	fixSwipeSlow : function(dom){
		var that = this;
		if(!that.isIOS){
			return;
		}
		if(typeof dom != "undefined" && typeof dom != "string"){
			return;
		}
		//check看看是否页面body上是否已经有了这段样式信息，如果已经存在，直接跳过创建过程，避免页面多次调用滑动组件创建多个问题
		var cache  =  arguments.callee;
		if(cache.hasFixStyle){
			return;
		}
		dom = dom + ":before";
		var styleNode = document.createElement("style");
		styleNode.id = "widget_fix_swipe";
		...
		width: 1px;\
		height: 1px;\
		 -webkit-animation: slip_rotate 2s infinite linear;\
		opacity:0.1;\
		}\
		@-webkit-keyframes slip_rotate { 0% { -webkit-transform: rotate(0deg); }\
	    100% { -webkit-transform: rotate(360deg); } }';
		
		$("#widget_fix_swipe").html(cssStyle);
		cache.hasFixStyle = true;
	},
	_initCommon: function(core,param) { // 作用：初始化
		...
		//居然会多次调用
		that.isIOS = that.isIOS ? that.isIOS : (typeof param.isIOS == "undefined") ? false : param.isIOS;
		if(that.isIOS && typeof param.fixSlowStyle != "undefined"){
			if(param.fixSlowStyle === true){
				//默认都是挂在<body>元素下面
				that.fixSwipeSlow("body");
			}else if(typeof param.fixSlowStyle == "string"){
				that.fixSwipeSlow(param.fixSlowStyle);
			}
		}
